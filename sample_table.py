...
def main():
    ...
    parser.add_argument('-m', '--master',
        ...
        help="master sample table [default: C:\\Users\\user\Documents\LC2018\LC_2018.sample_table.master.csv]")
    parser.add_argument('-o', '--out_dir',
        default='C:\\Users\\user\Desktop\\',
        help="output directory [default: C:\\Users\\user\Desktop\]")
    parser.add_argument('-i', '--id',
        type=int,
        help="first sample ID number to be assigned to the new table. Ignores the last sample ID number found in the master table")
    parser.add_argument('-f', '--fields',
        dest='fields',
        default="CruiseID,PrivateID,ContainerType,SampleType,SampleQuantity,StorageMethod",
        type=parse_commas,
        help="comma-separated list of additional field names to include in the sample table [default: CruiseID, PrivateID, ContainerType, SampleType, SampleQuantity, StorageMethod]")
    ...
    print("Outputting table of sample IDs in {}".format(output), file=sys.stdout)
    ...
    with open(output, 'w') as out_h, open(args.master, 'a') as master_h:
        ...
        for sid in list(range(first_id, last_id)):
            out_h.write('LC{},{},{}{}\n'.format(str(sid).zfill(args.pad), assignee, current_date, ncomma))
            master_h.write('LC{},{},{}\n'.format(str(sid).zfill(args.pad), assignee, current_date))
